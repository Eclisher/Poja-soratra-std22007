...
public class SoratraController {
    ...
    public ResponseEntity<ListURL> convertSoratra(
            @PathVariable String id
    ) {
        try {
            String originalUrl = bucketComponent.presign(SORATRA_KEY + id + ".txt", Duration.ofMinutes(4)).toString();
            String transformedUrl = bucketComponent.presign(SORATRA_KEY + "UpperCase/" + id + ".txt", Duration.ofMinutes(4)).toString();
